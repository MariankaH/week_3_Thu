...
Bcs alpha is for everything whereas colour and fill depend on continent
...
```{r}
plastic_waste %>% 
  filter(plastic_waste_per_cap<2) %>% 
  ggplot(aes(x=plastic_waste_per_cap, y=mismanaged_plastic_waste_per_cap))+
  geom_point()
```
We filtered it by plastic waste per capita smaller than 2 to see the relationship without the outlier of Trinidad and Tobago, and see that there's somewhat of a positive correlation between plastic waste per capita and mismanaged plastic waste, although variance increases with increasing plastic waste so there's not a clear linear relationship.
...
plastic_waste %>% 
  filter(plastic_waste_per_cap<2) %>% 
  filter(total_pop < 1000000000) %>% 
  ggplot(aes(x=total_pop, y=plastic_waste_per_cap))+
  geom_point()

plastic_waste %>% 
  filter(plastic_waste_per_cap<2) %>% 
  filter(coastal_pop<100000000) %>% 
  filter(!is.na(coastal_pop)) %>% 
  ggplot(aes(x=coastal_pop, y=plastic_waste_per_cap))+
  geom_point()
```
Not much linear association between plastic waste per capita and total or coastal population.