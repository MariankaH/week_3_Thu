...
# Your code below:
library(tidyverse)
vaccine <- read.csv("data/world_vaccination.csv")
world_vaccination <- vaccine %>% 
...
world_vacc_plot <- world_vaccination %>% 
  ...
  geom_point()+
  labs(title="Percentage of people vaccinated each year") +
  xlab("Year")+
  ylab("Percentage vaccination")
```
...
```{r}
world_vacc_plot_2 <- world_vaccination %>% 
...
polio_regions_line <- polio %>% 
  ggplot(aes(x=yr, y=pct_vaccinated, colour=who_region, shape=who_region)) +
  geom_line() +
  labs(title = "Percentage of Polio vaccination per year")
```
...
world_vaccination %>% 
  ggplot(aes(x=yr, y=pct_vaccinated, colour=who_region, shape=who_region)) +
  geom_line() +
  labs(title = "Percentage of vaccination per year") +
  facet_wrap(~ vaccine)
```
